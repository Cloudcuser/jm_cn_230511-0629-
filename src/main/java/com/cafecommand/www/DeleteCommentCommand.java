...
public class DeleteCommentCommand implements Command{
	...
	public void excute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("#### deleteComment Command oper #####");
		...
		if(flag) {
			System.out.println("Delete Comment command reNo error");
			return;
		}
		else {
			int result=0;
				result = boxdao.deleteBox(boxdto);
			
			if(result != 0) {
				System.out.println("Delete comment command work ok");
			}
			else {
				System.out.println("Delete comment command work failed");
			}
		}
	}
}
