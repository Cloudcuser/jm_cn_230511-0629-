...
<c:choose>
	<c:when test="${myPriv eq 1}" >
		<c:set var="myPrivName" value="없음"/>
	</c:when>
	<c:when test="${myPriv eq 2 }">
		<c:set var="myPrivName" value="매니저"/>
 	</c:when>
 	<c:when test="${myPriv eq 4 }">
		<c:set var="myPrivName" value="부매니저"/>
 	</c:when>
 	<c:when test="${myPriv eq 8 }">
		<c:set var="myPrivName" value="이벤트 스탭"/>
 	</c:when>
 	<c:when test="${myPriv eq 16 }">
		<c:set var="myPrivName" value="게시판 스탭"/>
 	</c:when>
 	<c:when test="${myPriv eq 32 }">
		<c:set var="myPrivName" value="디자인 스탭"/>
 	</c:when>
</c:choose>
<body>
<div class="top">
	...
	<section class="lookMyInfoWindow">
		<section class="lookMyInfoTableWindow">
			<table id="lookMyInfoTable">
				...
				<tr>
					<td>별명</td><td>${sessionScope.myNickname }</td>
				</tr>
				<tr>
					<td>취미</td><td>${sessionScope.myHobby }</td>
				</tr>
				<tr>
					<td>카페 가입일</td><td>${sessionScope.myRegDate }</td>
				</tr>
				<tr>
					<td>최근 방문일</td><td>${sessionScope.myComeDate }</td>
				</tr>
				<tr>
					<td>계정 찾기 질문</td><td>${sessionScope.myQues }</td>
				</tr>
				<tr>
					<td>계정 찾기 정답</td><td>${sessionScope.myAns }</td>
				</tr>
				<tr>
					<td>스탭 권한</td><td>${myPrivName }</td>
				</tr>
				<tr>
					<td>방문 횟수</td><td>${sessionScope.myVisitCnt }</td>
				</tr>				
				<tr>
					<td colspan="2" id="lastTd">
						<a href="startMain.jsp"><input type="button" value="메인으로" /></a>
						<a href="cafe_pro_PWPass_info.jsp?mode=editInfo"><input type="button" value="정보 수정" /></a>
					</td>
				</tr>
