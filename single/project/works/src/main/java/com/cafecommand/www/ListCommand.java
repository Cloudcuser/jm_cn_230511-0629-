...
public class ListCommand implements Command{
	...
	private int calPagePosValues(HttpSession session, HttpServletRequest request,  int mode, MenutableInfoDAO dao) {
		...
		if(mode == 15) {
			...
			session.setAttribute("limitPostPrint", 15);
		...
		if(flag){
			...
			System.out.println("나머지 수:"+remainingNumber);
			...
			System.out.println("(리스트 사이즈 / 제한 수) +1:"+remainingNumber);
			if(curPagePos == 1) {
				startSelectPoint = listSize-(limitPostPrint-1);
				lastSelectPoint = listSize;
			}
			else if(curPagePos == lastPageIdx) {
				startSelectPoint = 1;
				lastSelectPoint = listSize - ( limitPostPrint * (curPagePos-1));
			}
			else {
				startSelectPoint = listSize - ( limitPostPrint * (curPagePos))+1;
				lastSelectPoint = listSize - ( limitPostPrint * (curPagePos-1));
			}
