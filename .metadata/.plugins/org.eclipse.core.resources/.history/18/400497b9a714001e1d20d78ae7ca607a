...
<head>
	...
	<link rel="stylesheet" type=text/css href="./css/showList.css?after">
...
<body>
<script src="./js/showList.js" charset="utf-8"></script>
...
<c:choose>
   ...
   <!--  <div style="clear:left"></div> -->
			 ...
			 <table id="showAllPostTable">
				<caption>전체 글 보기</caption>
					...
					<tr>
						...
						<!-- 삭제 글 보기 코드 -->
						<%-- <tr>
							<td colspan="7">[ 삭제 글 ]</td>
						</tr>
						<c:forEach var="dto" items="${list }">		
							<c:choose>
								<c:when test="${dto.writer eq 'del'}">
									<tr id="delPost">
										<td>${ dto.contentNo}</td>
										<td>${ dto.subTableTag }</td>
										<td>#</td>
										<td><a href="view.do?no=${dto.contentNo}">${ dto.title}</a>[${dto.comment}] </td>
										<td>${ dto.title}</td>
										<td>###</td>
										<td>#</td>
										<td>#</td>				
    ...
    <script>
		...
		$("#SkipLeftBtn").on("load",eventRegPageSkipLeftBtn);
		...
		$("#SkipRightBtn").on("load",eventRegPageSkipRightBtn);
